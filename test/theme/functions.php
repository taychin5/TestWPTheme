...
if ( ! function_exists( 'test_setup' ) ) :
	...
	function test_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => __( 'Primary', 'test' ),
				'menu-2' => __( 'Footer Menu', 'test' ),
			)
		);
...
function test_scripts() {
	wp_enqueue_style( 'test-style', get_stylesheet_uri(), array(), TEST_VERSION );
	wp_enqueue_script( 'test-script', get_template_directory_uri() . '/js/script.min.js', array(), TEST_VERSION, true );
 ...
 * @return array
 */
function test_tinymce_add_class( $settings ) {
	$settings['body_class'] = 'block-editor-block-list__layout';
	return $settings;
}
add_filter( 'tiny_mce_before_init', 'test_tinymce_add_class' );
